...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `avg_sale_price` | number | ✓ | 매매 평균 (원) |
| `sale_count` | number | ✓ | 매매 건수 |
| `avg_lease_deposit` | number | - | 전세 평균 보증금 |
| `lease_count` | number | - | 전세 건수 |
| `avg_rent_monthly` | number | - | 월세 평균 월세 |
| `avg_rent_deposit` | number | - | 월세 평균 보증금 |
| `rent_count` | number | - | 월세 건수 |
| `tier_final` | string | - | 크롤러 확장 단계 (예: `"T1"`) |
...
**중요**: `url_map`의 Supabase URL을 Code가 웹페이지에 그대로 참조합니다. 별도 이미지 복사·업로드 **없음**. 대법원 원본 JPG(`images/photos/*`)는 감사·백업·재발행용으로만 보관.
...
**선택**. Cowork가 편의상 post.md 최상단에 YAML frontmatter를 담을 수 있으나 **Code는 `meta.json`을 우선**합니다. Frontmatter와 meta.json이 충돌하면 meta.json이 이김.

Cowork가 Frontmatter를 포함한다면 권장 필드:
- `title`, `summary`, `caseNumber`, `coverImage`, `publishedAt`, `tags`, `seoTags`

**필수 없음**. v2의 13개 필수 필드 요구는 폐기.
...
대법원 원본 JPG. **웹페이지에 사용되지 않음** — 웹페이지는 `meta.json.photos.url_map`의 Supabase URL을 참조합니다. 본 폴더의 파일은 감사·백업·재발행·오프라인 검토용.
...
`meta.json` 문자열 필드 전체 + `post.md` 본문 전체에서 `네이버` 리터럴 감지 시 abort. 마스킹 표기(`네○○`, `네ㅇㅇ`) 둘 다 허용.
...
2. `meta.json` 1개 존재
3. `post.md` 1개 존재
4. `data/pdf_text.txt`, `data/crawler_summary.json`, `data/photos_meta.json` 3개 존재
5. `meta.json`의 필수 필드(§3-1의 ✓) 전부 충족
6. `meta.json.photos.url_map`의 모든 URL이 HTTP 200 응답 가능(빌드 시 Code가 검증)
7. `post.md`에서 참조한 모든 `images/photos/*` 파일이 실제 존재
...
대법원 상세 API(`/pgj/pgj15B/selectAuctnCsSrchRslt.on`) 응답의 `csPicLst`가 0장인 사건에 대한 처리:

**동작**:
1. `seed-photos.mjs`는 csPicLst 0장 응답 시 **에러가 아닌 정상 종료**(exit 0).
2. `court_listings` 행에 다음을 마킹:
   - `photos = []` (빈 배열)
   - `photos_count = 0`
   - `photos_fetched_at` 현재 시각
   - `raw_snapshot.photos_unavailable = true`
   - `raw_snapshot.photos_unavailable_marked_at` ISO 8601
   - `raw_snapshot.photos_unavailable_reason` = `"csPicLst empty in selectAuctnCsSrchRslt.on at fetch time. Other channels not investigated."`

**의도**:
- 자동화 파이프라인이 무한 재시도 루프에 빠지지 않도록 마킹.
- "사건 데이터가 시스템에 없다"는 단정적 판정은 회피. 다른 채널(사설 사이트 캐시 등) 존재 가능성은 열어둠. 추가 추적은 ROI 낮아 보류.
- `photos.ts`의 캐시 hit 분기는 `photos_fetched_at`을 보고 재호출하지 않음.

**실측 사례 (2026-04-25)**:
- 부천지원 2024타경49993: `photos_unavailable=true` 마킹 후 정상 종료
- 서울남부지법 2023타경118641: 동일

### 10-3. Endpoint 룰 vs UI 룰 (독립적)

대법원 경매정보의 "검색 가능 여부"는 두 채널이 **독립적인 룰**을 가진다는 점이 2026-04-25 진단으로 확인됨.

| 채널 | 룰 |
|------|-----|
| **대법원 사이트 UI** (사용자가 브라우저로 검색) | 매각기일 기준 **D-14 이내**의 사건만 검색 노출 (사이트 정책) |
| **selectAuctnCsSrchRslt.on API** (우리 코드가 직접 호출) | **D-day 무관**. 사건마다 개별적으로 사진 등록 여부가 결정됨 |

**시사점**:
- UI에서 안 보인다고 API로도 안 받아오는 것은 아님 (예: 인천지법 527667 D-48은 UI 미노출이지만 API로 사진 8장 정상 수신).
- API에서 0장이라도 사이트의 "데이터 등록 부재"로 단정할 수 없음 (사이트의 검색 API도 D-14 기반이라 D-30+ 사건은 사이트 검색 결과로 검증 불가).
...
| photos_unavailable 처리 | 미정의 | §10-2 명시 (정상 종료 + 마킹) |
